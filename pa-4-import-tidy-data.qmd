...
youth <- read_csv("data-raw/YRBS2015.csv") # create original dataset named youth
```

Describe some of the issues you notice in the data:

> The columns have ... on all of them, many columns have nothing in it for some of the variables, and the variable names are on the second line instead.
...
```{r}
#| label: Fixing head
youth_fix_top <- read_csv("data-raw/YRBS2015.csv",
                          skip = 1) # Skipping the top row names
```
...
```{r}
#| label: fixing missing values
youth_fix_missing <- read_csv("data-raw/YRBS2015.csv",
                              skip = 1,
                              na = c(".", "N/A")) # fixing missing values using na function
```
...
```{r}
#| label: cleaning variables
youth_fix_variables <- read_csv("data-raw/YRBS2015.csv", 
                              skip = 1, 
                              na = c(".", "N/A"), #adjusting na's to "." or "N/A"
                              col_types = list(AgeCat = col_number(), #cleaning as col_number
                                               Grade = col_number(), #cleaning as col_number
                                               `Perception of weight` = col_factor(levels = c("Very underweight","Slightly underweight", "About the right weight", "Slightly overweight", "Very overweight")), #cleaning as col_factor
                                               Gender = col_character(), #cleaning as characters
                                               `Height in meters` = col_double(), #cleaning as double (numeric)
                                               `Weight in kilograms` = col_double(), #cleaning as double (numeric)
                                               `Body Mass Index` = col_double(), #cleaning as double (numeric)
                                               `BMI percentile` = col_double(), #cleaning as double (numeric)
                                               `Fruit eating` = col_factor(), #cleaning as a factor
                                               `Salad eating` = col_factor(), #cleaning as a factor
                                               `Other vegetable eating` = col_factor(), #cleaning as a factor
                                               `Soda drinking` = col_factor(), #cleaning as a factor
                                               `Breakfast eating` = col_number(), #cleaning as col_number
                                               `Physical activity >= 5 days` = col_number(), #cleaning as col_number
                                               `Television watching` = col_factor(), #cleaning as a factor
                                               `Sports team participation` = col_factor(), #cleaning as a factor
                                               .default = col_skip() #cleaning as .default to col_skip
                                               ))
```
...
```{r}
#| label: Janitor clean
snake_case <- janitor::clean_names(youth_fix_variables) # Janitor cleaning
```
...
youth_clean <- snake_case # making snake_case into youth_clean
write_csv(youth_clean, file = "data_clean/youth_clean.csv") # saving youth clean csv 
```

Why do we keep the code for the data cleaning but set the code chunks to `eval: false`?

> Because we already did it in the previous code chunk and don't need to show it again.
...
> The three main variables that we are missing in the first chart are breakfast eating, physical activity, and soda drinking. The physical activity for 5 days and breakfast eating have the most missing values combined. 

How does the missingness of the data impact what variables you might choose to analyze? Why?

> We are more likely to choose variables with data that isn't missing due to have the most accurate analysis.
